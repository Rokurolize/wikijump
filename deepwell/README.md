...
This script makes several changes to the emitted files before finishing to account for missing features or bugs within `sea-orm-cli`.
...
* `types/` &mdash; Shared domain enums, request references, and other reusable types.
* Other modules are utilities, and are generally understandable by looking at their contents.

JSON-RPC methods are registered in `api.rs`, decoded and authorized in `endpoints/`, and implemented through reusable operations in `services/<name>/service.rs`. Database models live in `models/`, while transport-neutral request and output structures live beside their owning service in `services/<name>/structs.rs`.

### Compilation

This executable targets the latest stable Rust. At time of writing, that is `1.94.0`.
...
For corpus parser failures, use the read-only, process-isolated
[`render-replay` convergence action](../docs/deepwell-render-replay.md) before
rerunning the full runtime finalizer.
