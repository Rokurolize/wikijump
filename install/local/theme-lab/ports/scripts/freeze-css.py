...
class CacheCSS:
    def __init__(self, cache: Path, assets: Path, replacements: dict[str, str] | None = None, omissions: dict[str, str] | None = None) -> None:
        self.cache = cache
        self.assets = assets
    ...
    def build(self, source_text: str, source_url: str) -> tuple[str, dict]:
        ...
        return text, {"assets": list(self.asset_rows.values()), "missing": [{"url": u, "reason": r} for u, r in sorted(self.missing.items())], "imports": sorted(self.seen_imports), "pruned_fonts": self.pruned_fonts, "template_placeholders": self.template_placeholders, "asset_replacements": self.replacements, "omitted_assets": self.omitted_assets}
...
def main() -> int:
    ...
    engine = CacheCSS(args.cache, args.assets, replacements, omissions)
