...
The all-theme form reports the complete declaration review: `resolved-canonical`, `intentional-fallback`, `intentional-cascade`, and unresolved status counts. Every finding includes its status and rationale in the per-theme output; completion requires `maintenance_unresolved_findings: 0`. The canonicalizer only consolidates identical at-context + selector + property keys, preserves `!important` precedence, and retains the reviewed `display` and intrinsic-sizing fallback sequences. It does not claim to solve shorthand/longhand interactions, selector equivalence, custom-property consumption, or the full browser cascade. `prepare-maintainable-sources.mjs --check` independently verifies that the retained exact-key winners and extracted page CSS match the accepted candidate.

For day-to-day authoring, use the package's `maintenance/base.wikidot.txt` and `maintenance/jp-overrides.css` rather than editing the chronological campaign repair modules in `candidate.wikidot.source.txt`. The deterministic build writes the actual publishable page source to `maintenance/final.wikidot.txt`; its maintenance manifest binds the exact base and override hashes. `prepare-maintainable-sources.mjs --check` verifies the generated bytes, the comment-free active CSS token stream, and the ordered include list against the frozen accepted candidate. Historical `vN` labels remain in `maintenance/manifest.json` instead of being the primary source a future updater has to read.
...
The plan reports upstream inline-CSS rule changes and `localization_overlap_selectors`: selectors changed upstream that are also changed by the JP baseline, targeted by canonical JP overrides, present in the generated final source, or depended on by a flattened-CSS transform. `localization_overlap_sources` identifies each owning layer; transform overlap entries include the transform identifier and rationale. A declaration-only change is detected even when the selector name is unchanged.
...
The declaration cleanup reviewed **389** repeated declaration keys across all 34 packages. It removed **301** same-value repetitions and consolidated **147** conflicting historical declarations while preserving the exact-key winner. The remaining **38** conflicts are documented `display` and intrinsic-sizing fallbacks; the audit records their rationale as `intentional-fallback`. All review entries are machine-readable in each maintenance manifest, and the aggregate audit fails completion for missing rationale or any unresolved/runtime-proof status. No browser recapture was needed because the generator proves unchanged active CSS identity, unchanged ordered includes, and unchanged exact-key winners against the frozen candidate.
