...
The live-reference mode is read-only, begins at 0.25 requests per second, and requires an identity-bound persistent cache that includes documents. A repeated test replays exact cached URLs instead of requesting Wikidot again. Candidate mode requires an exact non-443 identity, binds local screenshots to it, and writes a receipt accepted by the standing promotion controller only when every canary passes. DOMContentLoaded capture is an immediate DOM/CSS observation, not a compositor-filmstrip assertion.`;
}
