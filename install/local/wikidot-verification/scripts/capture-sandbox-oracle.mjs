  ...
  try {
    ...
    for (const [index, fixture] of registry.fixtures.entries()) {
      ...
      try {
        ...
        const liveCapture = await withTimeout(captureBrowserParityObservation({context: browser.context, url: liveResource.url, label: "live", index, outputDir: args.outputDir, contract, viewport: args.viewport, timeoutMs: args.timeoutMs, settleMs: args.settleMs}), `live capture ${fixture.fixture_id}`);
        console.log(JSON.stringify({fixture_id: fixture.fixture_id, phase: "capture-local"}));
        const localCapture = await withTimeout(captureBrowserParityObservation({context: browser.context, url: localResource.url, label: "local", index, outputDir: args.outputDir, contract, viewport: args.viewport, timeoutMs: args.timeoutMs, settleMs: args.settleMs}), `local capture ${fixture.fixture_id}`);
