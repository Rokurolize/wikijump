...
function parseArgs(argv) {
  ...
  for (; index < argv.length; index += 1) {
    ...
    if (arg === "--input") {
      ...
      args.assetFile = path.resolve(nextValue(arg));
    } else if (arg === "--headed") {
      args.headed = true;
    } else if (arg === "--help") {
      printHelpAndExit();
...
function printHelpAndExit() {
  console.log(
    "Usage: node install/local/wikidot-verification/scripts/browser-authoring-corpus-workflow.mjs --input canary-pages.tsv --output-dir DIR [--base-url URL] [--offset 0] [--limit 10] [--timeout-ms 45000] [--slug-prefix ui-corpus-] [--asset-file FILE] [--headed]",
  );
  process.exit(0);
}
...
async function readSource(row) {
  if (!row.source_path)
    throw new Error(`Missing source_path for ${row.slug || row.page_id}`);
...
async function waitForAction(page, expectedAction, timeoutMs) {
  await page.waitForFunction(
    (action) =>
      document
        .querySelector("[data-testid='lab-status']")
...
async function submitTagForm(page, editedTags, timeoutMs) {
  ...
  await page.getByTestId("tag-form").evaluate((form, value) => {
    ...
    if (!(input instanceof HTMLInputElement))
      throw new Error("tag editor input not found");
    input.value = value;
...
async function runWorkflow(browser, row, args, dirs, absoluteIndex) {
  ...
  try {
    source = await readSource(row);
    const response = await page.goto(labUrl, {
    ...
    const renderedResponse = await page.goto(renderedUrl, {
      waitUntil: "domcontentloaded",
      timeout: args.timeoutMs,
    });
    ...
    record(
      "open-rendered",
      Boolean(renderedResponse && renderedResponse.ok()) &&
        currentTextBytes > 0,
      `${renderedResponse?.status() ?? "no response"} bytes:${currentTextBytes}`,
    );
...
async function main() {
  ...
  await fs.writeFile(
    path.join(args.outputDir, "authoring-results.tsv"),
    [
  ...
  await fs.writeFile(
    path.join(args.outputDir, "authoring-summary.json"),
