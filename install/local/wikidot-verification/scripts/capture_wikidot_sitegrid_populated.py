...
FIXTURE_SCHEMA = "wikijump.open43.a1038_sitegrid_populated_fixture.v1"
ARTIFACT_SCHEMA = "wikijump.open43.a1038_sitegrid_populated_live_evidence.v1"
...
LANE_ROOT = Path("/var/tmp/pr1334-a1038-sitegrid-populated-evidence")
ALLOWLIST = {
    "install/local/wikidot-verification/scripts/capture_wikidot_sitegrid_populated.py",
    "install/local/wikidot-verification/fixtures/open43-a1038-sitegrid-populated.json",
    "install/local/wikidot-verification/artifacts/open43-a1038-sitegrid-populated-live.json",
    "install/local/wikidot-verification/tests/open43-a1038-sitegrid-populated-evidence.test.mjs",
}
...
def capture(fixture: dict[str, Any], fixture_path: Path) -> dict[str, Any]:
    ...
    with wikidot.Client() as client:
        client.connector.client.trust_env = False
        site = client.site.get("sandbox-for-codex")
        if site.domain != fixture["endpoint"]["host"]:
            raise RuntimeError(f"wikidot.py resolved an unexpected host: {site.domain}")
        for probe in fixture["probes"]:
            ...
            response = site.amc_request(body_fields)
    ...
    claims = claims_for(fixture, observations) if len(observations) == len(fixture["probes"]) else {rule_id: {"status": "blocked", "positive_observation_ids": [], "negative_observation_ids": [], "attempted_routes": ["anonymous edit/PagePreviewModule on sandbox-for-codex.wikidot.com"], "missing_authority": "The bounded capture did not complete all declared probes."} for rule_id in fixture["establishable_rule_ids"]}
