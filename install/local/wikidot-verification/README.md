...
`scripts/run-standing-browser-parity.mjs` defines the source-owned browser-parity receipt consumed by the maintained promotion chain. `verify-standing-candidate-parity-admission.mjs` binds that receipt to the candidate identity, live reference, and completion policy; `install/standing/scripts/verify-promotion-precondition.mjs` binds the admission to the sealed build, final-frozen inputs, and staging home before standing preparation or refresh. The runner has two intentional modes. `live-reference` captures only the six production-theme canaries from `scp-wiki.wikidot.com` through an identity-bound persistent response cache with a 0 ms fixed inter-request interval; repeated runs reuse retained responses, while cache misses still honor server `Retry-After`. `candidate` captures the same pages only from a sealed, expiring non-443 candidate and compares them to an exact sealed live reference. Neither mode targets port 443. Historical sealed references retain the request interval recorded when they were acquired and remain reusable evidence rather than forcing another external fetch.
