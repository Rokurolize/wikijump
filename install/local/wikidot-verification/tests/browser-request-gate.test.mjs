...
function createRoute(
  ...
  return {
    ...
    async fulfill(options) {
      actions.push({
        type: "fulfill",
        status: options.status ?? options.response?.status() ?? null,
      });
    },
  };
}
...
test("shared gate admits concurrent public requests one per four seconds", async () => {
  ...
  assert.deepEqual(
    grants.map((grant) => grant.released_at_epoch_ms),
    [0, 4_000, 8_000],
  );
  assert.deepEqual(clock.sleeps, [4_000, 4_000]);
  assert.deepEqual(
    gate.snapshot().grants.map((grant) => grant.sequence),
    [1, 2, 3],
  );
});
...
test("source and local contexts share the gate while only the exact local origin is exempt", async () => {
  ...
  await installBrowserRequestGate(localContext, {
    gate,
    exemptOrigins: ["https://scp-wiki.wikijump.localhost"],
  });

  const localExact = createRoute("https://scp-wiki.wikijump.localhost/scp-173");
  assert.equal(
    localContext.routes[0].pattern(new URL(localExact.request().url())),
    false,
  );
  localContext.events.get("request")(localExact.request());
  const source = createRoute("https://scp-wiki.wikidot.com/scp-173");
  const wrongPort = createRoute(
    "https://scp-wiki.wikijump.localhost:18443/scp-173",
  );
  assert.equal(
    localContext.routes[0].pattern(new URL(wrongPort.request().url())),
    true,
  );
  await Promise.all([
    sourceContext.routes[0].handler(source),
    localContext.routes[0].handler(wrongPort),
  ]);
  let connected = false;
  await sourceContext.webSocketRoutes[0].handler({
    connectToServer() {
      connected = true;
    },
  });
  ...
  assert.deepEqual(
    gate.snapshot().grants.map((grant) => grant.released_at_epoch_ms),
    [0, 4_000],
  );
...
test("the public gate admits Wikidot and css.wikidot.com but blocks unrelated public hosts before admission", async () => {
  ...
  const css = createRoute("https://css.wikidot.com/local--code/1", {
    resourceType: "stylesheet",
  });
  const page = createRoute(
    "http://sandbox-for-codex.wikidot.com/codex-oracle:fixture",
  );
  const styleFrame = createRoute(
    "https://interwiki.scpwiki.com/styleFrame.html?priority=1&theme=example",
    { resourceType: "document" },
  );
  const styleFrameAsScript = createRoute(
    "https://interwiki.scpwiki.com/styleFrame.html?priority=1&theme=example",
    { resourceType: "script" },
  );
  const styleFramePost = createRoute(
    "https://interwiki.scpwiki.com/styleFrame.html?priority=1&theme=example",
    { method: "POST", resourceType: "document" },
  );
  const interwikiRuntime = createRoute(
    "https://interwiki.scpwiki.com/interwiki.js",
    {
      resourceType: "script",
      frameUrl: "https://interwiki.scpwiki.com/interwikiFrame.html",
    },
  );
  const resizeRuntime = createRoute(
    "https://interwiki.scpwiki.com/resizeIframe.js",
    {
      resourceType: "script",
      frameUrl: "https://interwiki.scpwiki.com/interwikiFrame.html",
    },
  );
  const interwikiRuntimeFromMainPage = createRoute(
    "https://interwiki.scpwiki.com/interwiki.js",
    {
      resourceType: "script",
      frameUrl: "https://scp-wiki.wikijump.localhost/scp-173",
    },
  );
  const unknownInterwikiScript = createRoute(
    "https://interwiki.scpwiki.com/other.js",
    { resourceType: "script" },
  );
  ...
  assert.deepEqual(gate.snapshot().blocked_hosts, {
    "api.rlcdn.com": 1,
    "interwiki.scpwiki.com": 4,
  });
  assert.deepEqual(gate.snapshot().blocked_hosts_by_fixture, {
    "syntax-collapsible": { "api.rlcdn.com": 1, "interwiki.scpwiki.com": 4 },
  });
  ...
  assert.equal(
    isWikidotCapturePublicOrigin(
      "http://d3g0gp89917ko0.cloudfront.net/v--7690939296dc/common--javascript/WIKIDOT.combined.js",
    ),
    true,
  );
  assert.equal(
    isWikidotCapturePublicOrigin(
      "https://d3g0gp89917ko0.cloudfront.net/ads.js",
    ),
    false,
  );
  assert.deepEqual(styleFrameAsScript.actions, [
    { type: "abort", reason: "blockedbyclient" },
  ]);
  assert.deepEqual(styleFramePost.actions, [
    { type: "abort", reason: "blockedbyclient" },
  ]);
  assert.deepEqual(interwikiRuntime.actions, [{ type: "continue" }]);
  assert.deepEqual(resizeRuntime.actions, [{ type: "continue" }]);
  assert.deepEqual(interwikiRuntimeFromMainPage.actions, [
    { type: "abort", reason: "blockedbyclient" },
  ]);
  assert.deepEqual(unknownInterwikiScript.actions, [
    { type: "abort", reason: "blockedbyclient" },
  ]);
  assert.equal(
    isWikidotCapturePublicOrigin(
      "https://interwiki.scpwiki.com/styleFrame.html?priority=1",
      "document",
      "GET",
    ),
    true,
  ...
  assert.equal(
    isWikidotCapturePublicOrigin(
      "https://interwiki.scpwiki.com/interwikiFrame.html?lang=en",
      "document",
      "GET",
    ),
    true,
  ...
  assert.equal(
    isWikidotCapturePublicOrigin(
      "https://interwiki.scpwiki.com/styleFrame.html",
      "script",
      "GET",
    ),
    false,
  ...
  assert.equal(
    isWikidotCapturePublicOrigin(
      ...
      "script",
      "GET",
      "https://interwiki.scpwiki.com/interwikiFrame.html",
    ),
    true,
  );
  assert.equal(
  ...
  assert.equal(
    isWikidotCapturePublicOrigin(
      "https://interwiki.scpwiki.com/other.html",
      "document",
      "GET",
    ),
    false,
  ...
  assert.equal(
    isWikidotCapturePublicOrigin(
      "http://interwiki.scpwiki.com/styleFrame.html",
      "document",
      "GET",
    ),
    false,
...
test("theme dependencies are admitted by resource type while third-party execution is blocked", async () => {
  ...
  const handler = context.routes[0].handler;
  const stylesheet = createRoute(
    "https://cdn.scpwiki.com/theme/en/sigma/theme.css",
    { resourceType: "stylesheet" },
  );
  const font = createRoute(
    "https://cdn.scpwiki.com/theme/en/sigma/font.woff2",
    { resourceType: "font" },
  );
  const image = createRoute("https://cdn.scpwiki.com/theme/en/sigma/logo.svg", {
    resourceType: "image",
  });
  const script = createRoute(
    "https://cdn.scpwiki.com/theme/en/sigma/theme.js",
    { resourceType: "script" },
  );
  ...
  assert.deepEqual(script.actions, [
    { type: "abort", reason: "blockedbyclient" },
  ]);
...
test("a URL-only source response cache bypasses revalidation and request-context responses", async (t) => {
  const cases = [
    ...
    [
      "language variance",
      { "cache-control": "public, max-age=600", vary: "Accept-Language" },
    ],
    [
      "response cookie",
      {
...
test("documents and no-store assets keep using the unchanged request gate", async () => {
  ...
  await installBrowserRequestGate(context, { gate, responseCache });
  const handler = context.routes[0].handler;
  const document = createRoute("https://example.test/page", {
    resourceType: "document",
  });
  const noStoreUrl = "https://example.test/dynamic.js";
  const noStoreResponse = createFetchResponse({
...
test("intentional unsupported-request attribution requires the exact request object", async () => {
  ...
  assert.deepEqual(dataUrl.actions, [
    { type: "abort", reason: "blockedbyclient" },
  ]);
...
test("gate implementation failures latch enforcement closed", async () => {
  ...
  await gate.acquire();
  const blockedAfterSleepFailure = createRoute(
    "https://scp-wiki.wikidot.com/queued",
  );
  await handler(blockedAfterSleepFailure);
  assert.deepEqual(blockedAfterSleepFailure.actions, [
    { type: "abort", reason: "blockedbyclient" },
  ]);
...
test("a persisted gate prevents a later capture process from granting before the prior interval", async () => {
  const root = await fs.mkdtemp(
    path.join(os.tmpdir(), "wikijump-browser-request-state-"),
  );
...
test("only canonical Wikijump local origins can become local exemptions", () => {
  assert.deepEqual(
    localBrowserCaptureOrigins(
      "https://scp-wiki.wikijump.localhost/scp-173?x=1",
    ),
    [
  ...
  assert.throws(
    () => localBrowserCaptureOrigins("https://public.example/scp-173"),
    /\.wikijump\.localhost/,
  );
  assert.throws(
    () =>
...
test("capture lock refuses a live owner regardless of state confirmation", async () => {
  const root = await fs.mkdtemp(
    path.join(os.tmpdir(), "wikijump-browser-request-lock-"),
  );
...
test("capture lock safely replaces a sealed stale owner", async () => {
  ...
  const replacement = await acquireBrowserCaptureLock({
    lockPath,
    runId: "replacement",
    hostname: "test-host",
    processStartTicks,
...
test("capture lock replaces an unsealed stale owner when durable state preserves the request floor", async () => {
  ...
  const clock = createClock();
  const gate = await createPersistentBrowserRequestGate({
...
test("capture lock refuses an unsealed stale owner when durable state is unavailable or malformed", async (t) => {
  for (const state of [null, "malformed\n"]) {
    await t.test(
      state === null ? "missing state" : "malformed state",
      async () => {
        const root = await fs.mkdtemp(
          path.join(os.tmpdir(), "wikijump-browser-request-lock-"),
        );
        const lockPath = path.join(root, "campaign.lock");
        const processStartTicks = async (pid) =>
          pid === process.pid ? "123" : null;
        await fs.writeFile(
          lockPath,
          `${JSON.stringify({
            schema: "wikijump_full_parity.browser_capture_lock.v1",
        ...
        if (state !== null)
          await fs.writeFile(`${lockPath}.state.json`, state, { mode: 0o600 });
        ...
        await assert.rejects(
          ...
          (error) => {
            ...
            if (state === null) {
              ...
              assert.match(
                error.cause.message,
                /browser request gate state is malformed/,
              );
            }
            return true;
          },
