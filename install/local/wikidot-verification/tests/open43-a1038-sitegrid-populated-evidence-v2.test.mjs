...
const previousArtifactPath = "install/local/wikidot-verification/artifacts/open43-a1038-sitegrid-populated-live.json";
...
const lanePaths = [scriptPath, fixturePath, artifactPath, "install/local/wikidot-verification/tests/open43-a1038-sitegrid-populated-evidence-v2.test.mjs"];
const sha256 = (value) => crypto.createHash("sha256").update(value).digest("hex");
const readJson = (file) => JSON.parse(fs.readFileSync(path.join(root, file), "utf8"));

function validateFixture(fixture) {
  assert.equal(fixture.schema, "wikijump.open43.a1038_sitegrid_populated_fixture.v2");
  assert.deepEqual(fixture.surface_ids, surfaces);
  assert.equal(fixture.integration_base, base);
  assert.deepEqual(fixture.lane_paths, lanePaths);
  ...
  assert.deepEqual(fixture.missing_site_identities, ["wj-open43-sitegrid-missing-a-v2-20260810", "wj-open43-sitegrid-missing-b-v2-20260810"]);
...
test("frozen SiteGrid evidence obeys the bounded public-read contract", () => {
  ...
  assert.equal(artifact.schema, "wikijump.open43.a1038_sitegrid_populated_live_evidence.v2");
  ...
  assert.deepEqual(artifact.post_capture_fixture_normalization, {
    capture_rerun: false,
    new_sha256: "f9ca5c73aa72066c646ce22beca1a0dbfc8b3a0743372249ca39d49577800c19",
    normalization: "trailing_blank_line_only",
    old_sha256: "825aa0d7b78a69bdbb88fddef526db13c4c0f314d08c79e08e8ddfb6323f593c",
  });
  assert.equal(sha256(Buffer.concat([fixtureBytes, Buffer.from("\n")])), artifact.post_capture_fixture_normalization.old_sha256);
  assert.equal(artifact.post_capture_fixture_normalization.new_sha256, artifact.fixture_sha256);
  assert.deepEqual(artifact.counters, {
    actual_requests: 8,
    elapsed_seconds: 39.873103,
    pagepreview_attempts: 8,
    request_bytes: 1544,
    response_bytes: 10659,
    retries: 0,
  });
  assert.deepEqual(artifact.endpoint_identity, fixture.endpoint);
  const previousArtifactBytes = fs.readFileSync(path.join(root, previousArtifactPath));
  const previousArtifact = JSON.parse(previousArtifactBytes.toString("utf8"));
  assert.deepEqual(artifact.supersedes, {
    ...
    acquisition_status: previousArtifact.acquisition_status,
    case_disposition: previousArtifact.case_disposition,
    actual_requests: previousArtifact.counters.actual_requests,
    blocker: previousArtifact.blockers[0],
    reason: "repository-pinned environment is now available",
  });
  assert.equal(sha256(previousArtifactBytes), artifact.supersedes.artifact_sha256);
  assert.equal(previousArtifact.schema, "wikijump.open43.a1038_sitegrid_populated_live_evidence.v1");
  assert.equal(previousArtifact.acquisition_status, "blocked");
  assert.equal(previousArtifact.case_disposition, "non_closure");
  assert.equal(previousArtifact.counters.actual_requests, 0);
  assert.deepEqual(previousArtifact.blockers, ["pinned-python-environment-unavailable"]);
  ...
  if (artifact.acquisition_status === "captured") {
    assert.deepEqual(artifact.dependency_identity, {
    ...
    assert.equal(artifact.dependency_identity, null);
  }
  ...
  for (const [id, observation] of Object.entries(artifact.observations)) {
    ...
    if (!observation.rejected_oversize && !observation.request_failure) {
      ...
      assert.ok(observation.inert_image_descriptors.every((descriptor) => typeof descriptor.src === "string"));
    }
  }
