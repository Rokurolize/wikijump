...
class FakeRpc {
  constructor({after = page("ftml [22222222]; deepwell-render/v11")} = {}) {
    this.after = after;
    this.calls = [];
...
test("saved-page rerender preserves source and revision while updating the compiler", async () => {
  ...
  assert.equal(receipt.pages[0].after.compiled_generator, "ftml [22222222]; deepwell-render/v11");
  assert.deepEqual(
...
test("saved-page rerender rejects a stale compiler after rerender", async () => {
  await assert.rejects(
    rerenderSavedPageRuntime({
      ...
      administratorEmail: "admin@example.test",
      administratorPassword: "secret",
      rpcClient: new FakeRpc({after: page("ftml [11111111]; deepwell-render/v11")}),
    }),
