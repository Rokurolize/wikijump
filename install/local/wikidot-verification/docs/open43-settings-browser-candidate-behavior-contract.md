...
The permission matrix records the served action status and the next public settings read, including the exact site identity and settings revision. Anonymous, non-administrator, cross-site, stale-revision, wrong-origin, and expired-session requests change neither values nor revision. The adapter records the framework's JSON 403 transport rejection only for the deliberately wrong origin; it does not relax the action-result envelope for same-origin requests. The expired session returns 401 rather than 500. The administrator request changes only the fixed description and advances the revision exactly once.
...
`S758_CREATE_INITIAL` and `S758_CREATE_SETTLED` remain outside this reversible Settings CaseSet because successful create advances a monotonic category allocator that no public cleanup authority can restore. The separate `open43-settings-lifecycle` CaseSet runs them against the exact non-standing candidate after the producer proves the fresh disabled allocator precondition; its parent run must discard the entire candidate stack with `stop-promotion-candidate.mjs` and retain the passing disposal receipt. The blocked collision/delete/overflow/re-enable and category-add warning lifecycles remain live-authority rows and are not inferred from this candidate run. The blocked external analytics beacon contract, unobserved built-in theme mapping, unobserved bottom-toolbar DOM, the welcome-page effect, and autocomplete keyboard, focus, loading, error, success, cancel, and navigation frames also remain outside this CandidateCaseSet.
