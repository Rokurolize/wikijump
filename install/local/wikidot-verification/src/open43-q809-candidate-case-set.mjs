...
class Open43Q809Run {
  ...
  constructor({ session, fixture, resources }) {
  ...
  async execute() {
    ...
    return [{ case_id: OPEN43_Q809_CASE_IDS[0], observations: { anonymous_before: anonymousBefore, editor_before: editorBefore, anonymous_after: anonymousAfter } }];
  }
...
function verifyCase(caseId, observations, fixture) {
  if (caseId !== OPEN43_Q809_CASE_IDS[0]) throw new Error(`unknown Q809 case: ${caseId}`);
...
export function createOpen43Q809CandidateCaseSet({ sessionFactory = (options) => new CandidateHttpSession(options) } = {}) {
  return Object.freeze({
    ...
    prepareRun({ candidateIdentity, privateInput, signal, resources }) {
