...
export const OPEN43_SIMPLETODO_CASE_IDS = Object.freeze([
  "A1037_SIMPLETODO_READ_ONLY_SERVED",
]);

const SITE_SLUG = "scpaiueouiuiuiui";
const SITE_HOST = `${SITE_SLUG}.wikijump.localhost`;
const PAGE_SLUG_PREFIX = "open43-simpletodo-runtime";
const LIVE_EVIDENCE = Object.freeze({
  path: "install/local/wikidot-verification/artifacts/simpletodo-sendinvitations-live-preview.json",
  sha256: "d26c44a0de21437a0c344244f0b7d24778a5e66b5c24bb1839dfe546ec2df095",
  case_ids: Object.freeze([
    "simpletodo-missing-id",
    "simpletodo-empty-id",
    "simpletodo-id-initial",
  ]),
});
const MISSING_ID_HTML = '<div class="error-block">The SimpleTodo module must have an id.</div>';
const NORMAL_LABEL_HTML = '<div class="label">codex-live-probe</div>';
const ESCAPED_LABEL_HTML = '<div class="label">&lt;script&gt;saved&lt;/script&gt;</div>';
const EDIT_PERMISSION_HTML = '<span id="simpletodo-data-edit-permission">false</span>';
...
function sourceFor(marker) {
  return [
    ...
    '[[module SimpleToDo id="<script>saved</script>"]]',
    `AFTER ${marker}`,
  ].join("\n");
}
...
function requireCandidateSite(candidateIdentity) {
  ...
  if (endpoint.host !== SITE_HOST || endpoint.port === 443 || candidateIdentity.candidate.port_443_published !== false) {
    throw new Error(`Open43 SimpleToDo case requires an exact sealed non-standing ${SITE_HOST} candidate`);
  }
}
...
function requireCount(body, fragment, expected, name) {
  const actual = count(body, fragment);
  if (actual !== expected) throw new Error(`${name} has ${actual} occurrences of ${fragment}; expected ${expected}`);
}

export function assertSimpletodoReadOnlyBody(value, name, marker) {
  const body = requireNonEmptyString(value, `${name} body`);
  requireNonEmptyString(marker, "SimpleToDo run marker");
  for (const boundary of [`BEFORE ${marker}`, `AFTER ${marker}`]) {
    requireCount(body, boundary, 1, name);
  }
  requireCount(body, MISSING_ID_HTML, 2, name);
  requireCount(body, '<div class="error-block">', 2, name);
  requireCount(body, '<div class="simpletodo-box"', 2, name);
  requireCount(body, '<div class="simpletodo-sub-box"', 2, name);
  requireCount(body, '<div class="title">Here is a place for your title</div>', 2, name);
  requireCount(body, '<span class="text">Click me to edit !</span>', 2, name);
  requireCount(body, '<span class="text">Drag me !</span>', 2, name);
  requireCount(body, 'aria-disabled="true"', 4, name);
  requireCount(body, EDIT_PERMISSION_HTML, 2, name);
  for (const expected of [
    '<div class="simpletodo-box" id="simpletodo_0">',
    '<div class="simpletodo-sub-box" id="simpletodo_sub_0">',
    '<div class="simpletodo-box" id="simpletodo_1">',
    '<div class="simpletodo-sub-box" id="simpletodo_sub_1">',
    NORMAL_LABEL_HTML,
    ESCAPED_LABEL_HTML,
  ]) {
    requireCount(body, expected, 1, name);
  }
  const lowerBody = body.toLowerCase();
  for (const activeMarker of ACTIVE_MARKERS) {
    if (lowerBody.includes(activeMarker)) throw new Error(`${name} exposes active content ${activeMarker}`);
  }
  return body;
}

function bodyEvidence(value, name, marker) {
  const body = assertSimpletodoReadOnlyBody(value, name, marker);
  return {
...
function requestEvidence(events) {
  if (!Array.isArray(events)) throw new Error("SimpleToDo candidate request evidence is not an array");
  return events.map(({ service, operation, method, response_status }) => ({ service, operation, method, response_status }));
}

class Open43SimpletodoRun {
  #session;
  #resources;
  #pageSlug;
  #marker;
  ...
  async execute() {
    ...
    if (await this.#page() !== null) throw new Error("run-owned SimpleToDo page namespace already exists");
    ...
    const previewEvidence = bodyEvidence(preview?.body, "SimpleToDo preview", this.#marker);
    ...
    const created = await this.#rpc("page_create", {
      ...
      tags: [],
      revision_comments: "Open43 SimpleToDo read-only candidate",
    });
    if (!Number.isSafeInteger(created?.page_id) || !Number.isSafeInteger(created.revision_id) || created.slug !== this.#pageSlug) {
      throw new Error("page_create did not return a public SimpleToDo page identity");
    }
    this.#ownedPage = {
    ...
    if (!this.#matchesOwnedPage(saved)) throw new Error("saved SimpleToDo page does not match its public ownership proof");
    const savedEvidence = bodyEvidence(saved.compiled_body_html, "SimpleToDo saved view", this.#marker);
    return [{
      case_id: "A1037_SIMPLETODO_READ_ONLY_SERVED",
      observations: {
  ...
  async cleanup() {
    ...
    if (failures.length > 0) throw new AggregateError(failures, "SimpleToDo public cleanup failed");
...
function verifyCase(caseId, observations) {
  ...
  if (caseId !== "A1037_SIMPLETODO_READ_ONLY_SERVED") throw new Error(`unsupported Open43 SimpleToDo case: ${caseId}`);
  for (const name of ["preview", "saved"]) {
    requirePlainObject(observations[name], `${caseId} ${name}`);
    if (
      observations[name].read_only !== true
      || observations[name].missing_id_errors !== 2
      || observations[name].read_only_lists !== 2
    ) throw new Error(`${caseId} ${name} did not prove the SimpleToDo read-only contract`);
  }
...
function verifyCleanup(proof, resources) {
  if (proof?.page_get !== null) throw new Error("SimpleToDo cleanup did not prove public page absence");
  if (!Array.isArray(resources) || resources.length !== 1 || resources.some((resource) => resource.released !== true)) {
    throw new Error("SimpleToDo cleanup did not release its recorded page");
  }
  return { public_absence_verified: true, page_absent: true, resource_count: resources.length };
}

export function createOpen43SimpletodoCandidateCaseSet({
  ...
  const sourceFiles = Object.freeze([
    ...
    "install/local/wikidot-verification/src/open43-simpletodo-candidate-case-set.mjs",
  ...
  return Object.freeze({
    id: "open43-simpletodo-read-only",
    caseIds: OPEN43_SIMPLETODO_CASE_IDS,
    prepareRun({ runId, candidateIdentity, privateInput, signal, resources }) {
      ...
      const execution = new Open43SimpletodoRun({ session, resources, pageSlug: runSlug });
      return Object.freeze({
        sourceFiles,
        runtimeBindings: session.requiredServiceBindings,
        privateInputIdentity: session.privateInputIdentity,
        plan: {
          schema: "wikijump.open43_simpletodo_candidate_plan.v1",
          site_slug: SITE_SLUG,
          page_slug: runSlug,
          ...
          excluded_scope: ["SimpleToDo mutation", "browser lifecycle"],
        },
