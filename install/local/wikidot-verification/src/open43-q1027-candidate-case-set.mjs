...
class Open43Q1027Run {
  ...
  async #create(entry) {
    ...
    if (!this.#matches(entry, page) || !Number.isSafeInteger(page.revision_id)) throw new Error(`Q1027 ${entry.role} page_create did not return its exact public identity`);
    if (!this.#matches(entry, await this.#getPage(entry))) throw new Error(`Q1027 ${entry.role} page_get did not confirm its exact public identity`);
