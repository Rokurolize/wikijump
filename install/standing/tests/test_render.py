...
class RenderStandingConfigTest(unittest.TestCase):
    ...
    def test_materializes_identity_bound_compose_home(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_dir:
            ...
            self.assertIn("wikijump-standing-postgres-data", compose)
            self.assertIn("wikijump-standing-files-data", compose)
            self.assertIn("wikijump-standing-cache-data", compose)
            self.assertNotIn("runtime50x", compose)
