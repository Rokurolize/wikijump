...
* **Using GitHub for CI/CD.** The primary lock-in here comes from GitHub Actions for status, image build/publish, and deployment automation. Validation tests intentionally run only in the maintained local WSL workspace rather than on disposable GitHub runners. GitHub Container Registry is used for Minio image distribution.
