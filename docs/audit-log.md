...
| `file.undelete`       | `user_id`, `site_id`, `page_id`   | File ID       | Revision ID   |                   |                   |                   | `page_id` is the file's resulting parent page and revision ID is the resurrection revision. The registered `file_restore` request supplies the recorded IP. The event is emitted exactly once after a successful restore and participates in the request transaction; denied, failed, and repeated restores emit no event. |
