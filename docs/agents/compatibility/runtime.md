...
After activation, take fresh standing evidence bound to the merge identity. Candidate PASS is never standing PASS. Use the maintained standing comparison contract, exact browser/cache identity required by the retained reference, zero-repeat external replay, and the required full-page/canary checks.
